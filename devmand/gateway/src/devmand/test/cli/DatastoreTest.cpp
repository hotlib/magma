...
// report its data as invalid)
            ...
            TEST_F(DatastoreTest, diffMultipleOperations) {
                ...
                Path p1(
                        "/openconfig-interfaces:interfaces/openconfig-interfaces:interface/openconfig-interfaces:state");
                Path p2(
                        "/openconfig-interfaces:interfaces/openconfig-interfaces:interface/openconfig-interfaces:state/counters");
                ...
                for (const auto &multi : multimap) {
                    MLOG(MINFO)
                    << "key: " << multi.first << " handles: " << multi.second.path << " (type: " << multi.second.type
                    << ")" << " after: " << toPrettyJson(multi.second.after);
                }
