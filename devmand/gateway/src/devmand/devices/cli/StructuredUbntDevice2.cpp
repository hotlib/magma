...
static void parseConfig(
  ...
  channel.executeRead(cmd)
      .via(folly::getCPUExecutor().get())
      .thenValue([ifcId, cfg](auto output) {
        cfg->name = ifcId;
        ...
        parseLeaf<IfcType>(
            output, type, cfg->type, 1, [&ifcId](auto str) -> IfcType {
              if (str.find("lag") == 0) {
...
//  channel.executeRead(cmdState)
//      .via(folly::getCPUExecutor().get())
//      .thenValue([ifcId, state, cfg, &channel](auto stateOut) mutable {
//        const auto regexIfcState = regex(ifcId +
//        R"(\s+(\S+)\s+(\S+)\s*(.*))");
//
...
//          channel.executeRead(cmdStateEth)
//              .via(folly::getCPUExecutor().get())
//              .thenValue([state](auto outputStateEth) {
...
//  channel.executeRead(cmd)
//      .via(folly::getCPUExecutor().get())
//      .thenValue([eth, ifcId](auto output) {
...
static SemiFuture<shared_ptr<Ifcs>> parseIfcs(Channel& channel) {
  auto cmd = ReadCommand::create("show interfaces description");
  return channel.executeRead(cmd)
      .via(folly::getCPUExecutor().get())
      .thenValue([&channel](auto output) {
        ...
        return dynamic_pointer_cast<Entity, Ifcs>(interfaces);
      })
      .semi();
}
...
//  \"(.+)\")"); parseLeaf<string>(vlanCfgOut, vlanNameRegx,
//  vlan->config->name); vlan->config->status = Vlan::Config::Status::ACTIVE;
//
...
// static SemiFuture<shared_ptr<Nis>> parseNetworks(Channel& channel) {
//  parseDefaultNetwork(channel)
//  .via(folly::getCPUExecutor().get())
//  .thenValue([](auto defaultNi) {
...
StructuredUbntDevice::StructuredUbntDevice(
    ...
    : Device(application, id_, false), channel(_channel), cmdCache(_cmdCache) {}
...
shared_ptr<State> StructuredUbntDevice::getState() {
  ...
  vector<SemiFuture<shared_ptr<Entity>>> requests;
  requests.emplace_back(parseIfcs(*channel));
  // TODO make parse Networks also asynchronous
  // TODO test
  // TODO make this code safe e.g. race conditions between destructor and
  // callbacks
  //  requests.emplace_back(parseNetworks(*channel));

  SemiFuture<vector<shared_ptr<Entity>>> futureOfRequests =
      folly::collectSemiFuture(requests.begin(), requests.end());

  state->addRequest(
      move(futureOfRequests)
          .via(folly::getCPUExecutor().get())
          .thenValue([state](auto roots) {
            // TODO the conversion here is: Object -> Json -> folly:dynamic
            // the json step is unnecessary
            auto& bundle = mreg->getBundle(Model::OPENCONFIG_0_1_6);
            for (auto root : roots) {
              string json = bundle.encode(*root);
              folly::dynamic dynamicRoot = folly::parseJson(json);
              state->setStatus(true);
              state->update([&dynamicRoot](folly::dynamic& lockedState) {
                lockedState.merge_patch(dynamicRoot);
              });
            }
          })
          .thenError(
